    ...
    for c in inputs.queries_list:
        ...
        if c_elem.value not in inputs.facts_list and c_elem.undetermined:
            while clearing.lower() != 'y' and clearing.lower() != 'n':
                clearing = input(
                    f"{c} is undetermined. Would you like to set its status? (y/n) : ")
            if clearing.lower() == 'y':
                new_status = 'z'
                while new_status.lower() != 't' and new_status.lower() != 'f':
                    new_status = input(
                        f"Please provide a status for {c} (t/f) : ")
                if new_status == 't':
                    c_elem.status = TRUE
                elif new_status == 'f':
                    c_elem.status = FALSE
...
def ft_interactive(inputs, args):
    ...
    while change_facts.lower() != 'n':
        change_facts = input(
            'Would you like to change the initial facts? (y/n)  ')
        ...
        if change_facts.lower() == 'y':
            ...
            if new_facts == '':
                ...
                new_facts = '=' + \
                    new_facts if new_facts[0] != '=' else new_facts
